...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.layer2 = self._make_layer(block, block_inplanes[1], layers[1], spatial_dims, shortcut_type, norm=norm, act=act, stride=strides[0])
        self.layer3 = self._make_layer(block, block_inplanes[2], layers[2], spatial_dims, shortcut_type, norm=norm, act=act, stride=strides[1])
        self.layer4 = self._make_layer(block, block_inplanes[3], layers[3], spatial_dims, shortcut_type, norm=norm, act=act, stride=strides[2]) # stride=1 to match unet, org=2
        self.avgpool = avgp_type(block_avgpool[spatial_dims]) if feed_forward else None
