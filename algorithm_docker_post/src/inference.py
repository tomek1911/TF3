...
def memory_efficient_inference_with_overlap(
    ...
    Args:
        model: segmentation model (UNET configuration) returning (seg_multiclass, None).
        ...
        blend_mode: "gaussian" or "constant" weighting for blending.
        sigma_scale: used if blend_mode="gaussian"; sigma = sigma_scale * dim_size.

    Returns:
        multiclass_output: [1,1,H,W,D] uint8 (argmax), on device
    """
    ...
    # Preallocate accumulators — seg probs on CPU to save GPU memory
    ...
    with torch.no_grad():
        for s in slices:
            ...
            seg_multiclass, _ = model(patch)  # UNET: returns (logits, None)

            w_patch = importance_map[:, :, :seg_multiclass.shape[2], :seg_multiclass.shape[3], :seg_multiclass.shape[4]]
            weighted_probs = torch.softmax(seg_multiclass, dim=1).to(cast_dtype) * w_patch

            sum_weights[:, :, s[0], s[1], s[2]] += w_patch
            sum_probs[:, :, s[0], s[1], s[2]] += weighted_probs.cpu()
...
def run_inference(input_tensor, args, device, transform) -> np.ndarray:
    ...
    with torch.no_grad(), torch.amp.autocast(enabled=True, dtype=torch.float16, device_type=device.type):
        ...
        multiclass_segmentation = memory_efficient_inference_with_overlap(args, model, input_tensor['image'], args.patch_size, device=device, overlap=0.25,
                                                                            blend_mode="gaussian", sigma_scale=0.125, cast_dtype=torch.float16)
        del model
        gc.collect()
        torch.cuda.empty_cache()

    print("remapping centerF predictions to challenge labels (pulp is class 46 → 50)")
    with torch.no_grad():
        remapped = remap_labels_torch(multiclass_segmentation.squeeze().to(torch.int32, device), pred_to_challange_map)
        del multiclass_segmentation
        torch.cuda.empty_cache()

        prediction = transform.post_inference_transform({"pred": MetaTensor(remapped.unsqueeze(0)), "image": input_tensor["image"][0]})["pred"]
